...
#
# 06 Mar 2019:
# Update at function compose_mail for Reply-to
#
"""
...
def build_mail(text, html=None, attachments=[], embeddeds=[], replyto='', name_replyto=''):
    """
    ...
    if replyto:
        if name_replyto:
           replyto = u"%s <%s>" % (name_replyto, replyto)
        main.add_header('reply-to', replyto)
...
def compose_mail(sender, recipients, subject, default_charset, text, html=None, attachments=[], embeddeds=[], cc=[], bcc=[], message_id_string=None, date=None, headers=[], replyto='', name_replyto=''):
    ...
    """
    message=build_mail(text, html, attachments, embeddeds, replyto=replyto, name_replyto=name_replyto)
